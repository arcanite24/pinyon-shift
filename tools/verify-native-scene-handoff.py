...
if __name__ == "__main__":
    ...
    images = [pixels(output / f"track-source-{frame}.ppm")
              for frame in (range(5000, 5021) if continuous else (5000, 5001, 5002))]
    assert images[0] != images[0][:3] * (1280 * 720), "missing compatibility frame"
    for frame, image in enumerate(images[1:], 5001):
    ...
    print(f"native scene handoff: fallback and {len(images) - 1} claimed frames passed")
