...
def fixture(shared=3):
    ...
    summary = event(
        ...
        unjoined_scopes="2",
        submission_joins="12",
        command_context_observations="30",
...
class TrackModelRuntimeJoinTests(unittest.TestCase):
    ...
    def test_accepts_non_track_root_exclusions(self):
    ...
    def test_rejects_missing_prepared_command_join(self):
        events = copy.deepcopy(fixture())
        events[-1].update(
            status="incomplete",
    ...
    def test_rejects_unjoined_scopes(self):
        ...
        self.assertIn(
            "no exact scope joined an indirect command context",
            document["failures"],
        )
