...
struct Snr03SceneSnapshot {
  ...
  std::vector<Snr03VegetationItem> characters;
};
struct Snr03ManagerRecord {
  uint64_t next_direct = 0;
  uint32_t record = 0, source = 0, packet = 0;
  std::array<uint32_t, 4> record_words{};
  std::array<uint32_t, 3> source_words{};
};
struct Snr03ManagerScene {
  uint64_t source_frame = 0;
  uint32_t view = 0, camera = 0;
  std::array<uint32_t, 16> camera80{}, camera144{};
  std::vector<Snr03ManagerRecord> records;
};
...
struct Snr03CharacterPayload {
  struct Item {
    std::vector<uint8_t> vertices;
    std::vector<Snr03CharacterDraw> draws;
  };
  std::map<uint32_t, Item> by_packet;
  uint32_t bytes = 0;
  bool rejected = false;
};
...
std::map<uint64_t, Snr03CharacterPayload> snr03_character_payloads;
...
thread_local std::vector<Snr03ManagerRecord> snr03_manager_title_records;
...
struct Snr03CarScene {
  uint64_t source_frame = 0;
  uint32_t view = 0, camera = 0;
  std::array<uint32_t, 32> camera_matrices{};
...
std::map<uint64_t, Snr03RemainderPayload> snr03_remainder_payloads;
...
uint64_t Snr03Fingerprint(const Snr03SceneSnapshot& scene) {
  ...
  for (const auto& item : scene.characters) {
    add(item.record);
    add(item.vertex_descriptor);
    add(item.vertex_address);
    add(item.vertex_size);
...
void ObserveSnr03CharacterPayload(
  ...
  const auto item = std::find_if(scene->second->characters.begin(),
                                 scene->second->characters.end(), [&](const auto& row) {
    return row.packet_physical == observation.draw_packet_physical_address;
  });
  if (item == scene->second->characters.end()) return;
  auto& payload = snr03_character_payloads[uint64_t(target)];
  if (payload.rejected) return;
  const auto* fetch = observation.vertex_fetches;
  if (!observation.draw_sequence || observation.vertex_fetch_count != 1 || !fetch ||
      observation.vertex_shader_hash != 0xAC345DADF2F24AE4ull ||
      observation.pixel_shader_hash != 0xB77EC20EA53C20F8ull ||
      observation.guest_primitive_type != 13 || observation.index_buffer_type != 0 ||
      !observation.index_count || observation.index_count > 8192 ||
      observation.index_count % 4 ||
      observation.index_count * 4 != (item->vertex_size & 0x03FFFFFC) ||
      fetch[0].fetch_constant != 95 || fetch[0].stride_words != 4 ||
      fetch[0].guest_base != (item->vertex_address & 0x1FFFFFFC) ||
      fetch[0].length != observation.index_count * 4 ||
      fetch[0].length > 32768 ||
      fetch[0].cpu_snapshot_status != 1 || !fetch[0].cpu_snapshot_bytes ||
  ...
  auto [found, inserted] = payload.by_packet.try_emplace(item->packet_physical);
  auto& owned = found->second;
  if (inserted) {
    if (payload.by_packet.size() > 64 || payload.bytes > 512 * 1024 - fetch[0].length) {
      payload.rejected = true;
      return;
    }
    owned.vertices.assign(fetch[0].cpu_snapshot_bytes,
                          fetch[0].cpu_snapshot_bytes + fetch[0].length);
    payload.bytes += fetch[0].length;
  } else if (owned.vertices.size() != fetch[0].length ||
             !std::equal(owned.vertices.begin(), owned.vertices.end(),
                         fetch[0].cpu_snapshot_bytes)) {
    payload.rejected = true;
    return;
  }
  ...
  draw.vertex_count = observation.index_count;
  ...
  if (draw.packed.size() != observation.vertex_float_constant_count * 4 ||
      std::any_of(owned.draws.begin(), owned.draws.end(),
                  [&](const auto& row) { return row.sequence == draw.sequence; })) {
    payload.rejected = true;
    return;
  }
  REXGPU_INFO("FH1 SNR03 character prepared {{\"frame\":{},\"packet\":{},"
              "\"sequence\":{},\"count\":{},\"vertex_hash\":{},"
              "\"bitmap\":[{},{},{},{}],\"packed_hash\":{},"
              "\"specialization\":{}}}",
              observation.frame_sequence, item->packet_physical, draw.sequence,
              draw.vertex_count, fetch[0].cpu_snapshot_hash,
              draw.bitmap[0], draw.bitmap[1], draw.bitmap[2], draw.bitmap[3],
              Snr02HashWords(draw.packed), draw.specialization);
  owned.draws.push_back(std::move(draw));
}
...
void ObserveSnr03ManagerPayload(
  ...
  if (!observation.draw_sequence ||
      ...
      observation.index_count == 0 ||
      observation.index_count > 32768 ||
      observation.index_buffer_length != observation.index_count * 2 ||
      observation.vertex_fetch_count != 2 ||
      observation.vertex_fetch_capacity < 2 || !fetch ||
      fetch[0].fetch_constant != 95 || fetch[0].stride_words != 8 ||
      fetch[0].length > 128 * 1024 ||
      fetch[1].fetch_constant != 94 || fetch[1].stride_words != 3 ||
  ...
  REXGPU_INFO("FH1 SNR03 manager prepared {{\"frame\":{},\"sequence\":{},"
              "\"packet\":{},\"packed_hash\":{},\"specialization\":{}}}",
              observation.frame_sequence, draw.sequence, draw.packet,
              Snr02HashWords(draw.packed), draw.specialization);
...
void ObserveSnr03RemainderPayloadLocked(
  ...
  if (!observation.draw_sequence || !observation.vertex_fetches ||
      !observation.vertex_float_constant_bitmap ||
      !observation.vertex_float_constant_words ||
      observation.vertex_fetch_count == 0 ||
      observation.vertex_fetch_count > 3 ||
      ...
      !observation.index_buffer_length ||
      observation.index_buffer_length > 128 * 1024 ||
  ...
  draw.host_primitive = observation.host_primitive_type;
  ...
  if (observation.texture_fetch_count)
    draw.textures.assign(observation.texture_fetches,
...
void ObserveSnr03FinalDrawState(
  ...
  if (const auto manager_scene = snr03_manager_scenes.find(uint64_t(target));
    ...
    if (title != manager_scene->second->records.end()) {
      ...
      std::copy_n(observation.scissor, 4, draw.scissor.begin());
      draw.final_seen = true;
      REXGPU_INFO("FH1 SNR03 manager final {{\"frame\":{},\"sequence\":{},"
                  "\"packet\":{},\"system_hash\":{},\"fetch_hash\":{},"
                  "\"dynamic\":{},\"raster\":{},\"clip\":{},\"depth\":{}}}",
                  observation.frame_sequence, draw.sequence, draw.packet,
                  Snr02HashWords(draw.system), Snr02HashWords(draw.fetch),
                  draw.dynamic, draw.raster_mode, draw.clip_control,
                  draw.depth_control);
      return;
    }
  }
  if (auto payload_it = snr03_remainder_payloads.find(uint64_t(target));
      payload_it != snr03_remainder_payloads.end()) {
    auto& payload = payload_it->second;
    // ponytail: linear lookup is bounded to one probe frame; index by sequence
    // if the selected diagnostic grows beyond a few thousand draws.
    const auto found = std::find_if(payload.draws.begin(), payload.draws.end(),
                                    [&](const auto& row) {
      return row.sequence == observation.draw_sequence;
    });
    if (found != payload.draws.end()) {
      if (found->final_seen ||
          found->packet != observation.draw_packet_physical_address ||
          !observation.system_constant_words ||
          observation.system_constant_word_count < 64 ||
      ...
      for (const auto& fetch : draw.fetches) {
        if (fetch.constant >= 96 ||
            (observation.fetch_constant_words[fetch.constant * 2] &
                 0x1FFFFFFC) != fetch.range.first ||
            (observation.fetch_constant_words[fetch.constant * 2 + 1] &
                 0x03FFFFFC) != fetch.range.second) {
          payload.rejected = true;
          return;
        }
      }
      ...
      REXGPU_INFO("FH1 SNR03 remainder final {{\"frame\":{},"
                  "\"sequence\":{},\"packet\":{},\"family\":{},"
                  "\"system_hash\":{},\"fetch_hash\":{},"
                  "\"dynamic\":{},\"raster\":{},\"clip\":{},"
                  "\"depth\":{}}}", observation.frame_sequence,
                  draw.sequence, draw.packet, draw.family,
                  Snr02HashWords(draw.system),
                  Snr02HashWords(draw.bound_fetch), draw.dynamic,
                  draw.raster_mode, draw.clip_control, draw.depth_control);
      return;
    }
  }
  const auto scene = snr03_scenes.find(uint64_t(target));
  ...
  const auto character = std::find_if(
      scene->second->characters.begin(), scene->second->characters.end(),
      [&](const auto& row) {
        return row.packet_physical == observation.draw_packet_physical_address;
      });
  if (character != scene->second->characters.end()) {
    auto& payload = snr03_character_payloads[uint64_t(target)];
    const auto owned = payload.by_packet.find(character->packet_physical);
    if (owned == payload.by_packet.end()) { payload.rejected = true; return; }
    const auto state = std::find_if(owned->second.draws.begin(), owned->second.draws.end(),
                                    [&](const auto& row) {
      return row.sequence == observation.draw_sequence;
    });
    if (state == owned->second.draws.end() || state->final_seen ||
        !observation.system_constant_words ||
        observation.system_constant_word_count < 64 ||
        !observation.fetch_47_words || !observation.viewport ||
        !observation.scissor || !observation.bound_vertex_float_constant_words ||
        observation.bound_vertex_float_constant_count * 4 != state->packed.size() ||
        !std::equal(state->packed.begin(), state->packed.end(),
                    observation.bound_vertex_float_constant_words)) {
      payload.rejected = true;
      return;
    }
    ...
    std::copy_n(observation.scissor, 4, draw.scissor.begin());
    draw.final_seen = true;
    REXGPU_INFO("FH1 SNR03 character final {{\"frame\":{},\"packet\":{},"
                "\"sequence\":{},\"system_hash\":{},\"fetch_hash\":{},"
                "\"dynamic\":{},\"mode\":{},\"clip\":{},\"depth\":{}}}",
                observation.frame_sequence, character->packet_physical,
                draw.sequence, Snr02HashWords(draw.system),
                Snr02HashWords(draw.fetch), draw.dynamic, draw.raster_mode,
    ...
    if (const auto scene = snr03_scalar_scenes.find(observation.frame_sequence - 1);
      ...
      if (record != scene->second->records.end()) {
        ...
        REXGPU_INFO("FH1 SNR03 scalar title join {{\"frame\":{},"
                    ...
                    observation.index_buffer_type, valid);
        if (valid && !family) {
          family = record->caller == 0x82415A28 ? 2 : 3;
          title_key = record->packet;
        }
      }
    }
    if (family)
...
void ObserveSnr03ManagerOutputFrame(uint64_t output_frame) {
  std::shared_ptr<const Snr03ManagerScene> scene;
  Snr03ManagerPayload payload;
  {
    std::lock_guard lock(snr03_scene_mutex);
    if (const auto found = snr03_manager_scenes.find(output_frame - 1);
        found != snr03_manager_scenes.end()) {
      scene = std::move(found->second);
      snr03_manager_scenes.erase(found);
    }
    if (const auto found = snr03_manager_payloads.find(output_frame - 1);
        found != snr03_manager_payloads.end()) {
  ...
  for (const auto& draw : payload.draws) {
    valid &= draw.final_seen && packets.contains(draw.packet) &&
             sequences.insert(draw.sequence).second;
    drawn_packets.insert(draw.packet);
    for (uint32_t slot = 0; slot < 3; ++slot) {
      valid &= payload.ranges[slot].contains(draw.ranges[slot]);
    }
  }
  valid &= packets == drawn_packets;
  std::sort(payload.draws.begin(), payload.draws.end(),
  ...
  constexpr std::array<char, 8> magic{'S','N','R','0','3','M','1','\0'};
  write(magic);
  write(scene->source_frame);
  write(scene->view);
  write(scene->camera);
  write(uint32_t(scene->records.size()));
  write(uint32_t(payload.draws.size()));
  for (const auto& ranges : payload.ranges) write(uint32_t(ranges.size()));
  write(scene->camera80);
  write(scene->camera144);
  for (const auto& record : scene->records) {
    write(record.next_direct);
    write(record.record);
    write(record.source);
    write(record.packet);
    write(record.record_words);
    write(record.source_words);
  }
  for (const auto& ranges : payload.ranges) {
    for (const auto& [key, bytes] : ranges) {
      write(key.first);
      write(key.second);
      encoded.insert(encoded.end(), bytes.begin(), bytes.end());
  ...
  for (const auto& draw : payload.draws) {
    ...
    write(draw.shader);
    write(draw.pixel_shader);
    write(draw.specialization);
    write(draw.dynamic);
    write(draw.count);
    write(draw.host_index_format);
    write(draw.index_endianness);
    for (const auto& range : draw.ranges) {
      write(range.first);
      write(range.second);
    }
    for (const auto& texture : draw.textures) {
      write(texture.fetch_constant);
      write(texture.type);
      write(texture.base_address);
      write(texture.mip_address);
      write(texture.format);
      write(texture.dimension);
      write(texture.width);
      write(texture.height);
      write(texture.stack_depth);
    }
    write(draw.bitmap);
    write(uint32_t(draw.packed.size()));
    for (uint32_t word : draw.packed) write(word);
    write(draw.system);
    write(draw.fetch);
    write(draw.raster_mode);
    write(draw.clip_control);
    write(draw.depth_control);
    write(draw.viewport);
    write(draw.scissor);
...
void ObserveSnr03RemainderOutputFrame(uint64_t output_frame) {
  std::shared_ptr<const Snr03CarScene> car;
  std::shared_ptr<const Snr03ScalarScene> scalar;
  Snr03RemainderPayload payload;
  {
    std::lock_guard lock(snr03_scene_mutex);
    if (const auto found = snr03_car_scenes.find(output_frame - 1);
        found != snr03_car_scenes.end()) {
      car = std::move(found->second);
      snr03_car_scenes.erase(found);
    }
    if (const auto found = snr03_scalar_scenes.find(output_frame - 1);
        found != snr03_scalar_scenes.end()) {
      scalar = std::move(found->second);
      snr03_scalar_scenes.erase(found);
    }
    if (const auto found = snr03_remainder_payloads.find(output_frame - 1);
        found != snr03_remainder_payloads.end()) {
      payload = std::move(found->second);
      snr03_remainder_payloads.erase(found);
    }
  }
  bool valid = car && scalar && !payload.rejected && !payload.draws.empty() &&
               payload.draws.size() <= 4096 && payload.bytes <= 32 * 1024 * 1024;
  if (car && scalar)
    valid &= car->source_frame == scalar->source_frame &&
             car->source_frame + 1 == output_frame &&
             car->view == scalar->view && car->camera == scalar->camera;
  std::set<uint64_t> sequences;
  for (const auto& draw : payload.draws) {
    valid &= draw.final_seen && sequences.insert(draw.sequence).second &&
             payload.indices.contains(draw.indices);
    for (const auto& fetch : draw.fetches)
    ...
    if (draw.family == 1 && car) {
      ...
      valid &= std::any_of(scalar->records.begin(), scalar->records.end(),
                           [&](const auto& row) {
        return row.packet == draw.title_key &&
               (draw.family == 2) == (row.caller == 0x82415A28);
      });
    } else {
      valid = false;
    }
  ...
  if (valid) {
    ...
    write(car->view);
    write(car->camera);
    write(uint32_t(car->records.size()));
    write(uint32_t(scalar->records.size()));
    write(uint32_t(payload.draws.size()));
    write(uint32_t(payload.vertices.size()));
    write(uint32_t(payload.indices.size()));
    write(car->camera_matrices);
    for (const auto& row : car->records) {
      write(row.dispatch); write(row.target); write(row.words);
      write(row.owner); write(row.owner_vtable); write(row.input);
      write(row.owner_call); write(row.owner_args);
    }
    for (const auto& row : scalar->records) {
      write(row.direct_ordinal); write(row.scalar_ordinal);
      write(row.packet); write(row.caller); write(row.object);
      write(row.object_vtable); write(row.command); write(row.selector);
      write(row.input_count);
    }
    ...
    for (const auto& draw : payload.draws) {
      write(draw.family); write(draw.title_key); write(draw.sequence);
      write(draw.packet); write(draw.shader); write(draw.pixel_shader);
      write(draw.specialization); write(draw.dynamic);
      ...
      for (const auto& texture : draw.textures) {
        ...
        write(texture.base_address); write(texture.mip_address);
        write(texture.format); write(texture.dimension);
        write(texture.width); write(texture.height);
        write(texture.stack_depth);
      }
      write(draw.bitmap); write(uint32_t(draw.packed.size()));
      for (uint32_t word : draw.packed) write(word);
  ...
  REXGPU_INFO("FH1 SNR03 remainder owned scene output_frame={} "
              "car_records={} scalar_records={} draws={} bytes={} "
              "rejected={} fixture_bytes={} written={}", output_frame,
              car ? car->records.size() : 0,
...
void ObserveSnr03OutputFrame(uint64_t output_frame, void* device) {
  ...
  Snr03CharacterPayload characters;
  {
    std::lock_guard lock(snr03_scene_mutex);
    ...
    const auto character_it = snr03_character_payloads.find(output_frame - 1);
    if (character_it != snr03_character_payloads.end()) {
      characters = std::move(character_it->second);
      snr03_character_payloads.erase(character_it);
    }
  }
  if (!scene) {
  ...
  if (!scene->characters.empty()) {
    bool complete = !characters.rejected &&
                    characters.by_packet.size() == scene->characters.size();
    std::vector<char> encoded;
    auto write = [&](const auto& value) {
      const auto* data = reinterpret_cast<const char*>(&value);
      encoded.insert(encoded.end(), data, data + sizeof(value));
    };
    constexpr std::array<char, 8> magic{'S','N','R','0','3','C','1','\0'};
    write(magic);
    write(scene->source_frame);
    write(scene->view);
    write(scene->camera);
    write(uint32_t(scene->characters.size()));
    uint32_t draw_count = 0;
    for (const auto& [packet, item] : characters.by_packet)
      draw_count += uint32_t(item.draws.size());
    complete &= draw_count > 0 && draw_count <= 128;
    write(draw_count);
    write(scene->camera80);
    write(scene->camera144);
    std::set<uint64_t> sequences;
    for (const auto& item : scene->characters) {
      const auto found = characters.by_packet.find(item.packet_physical);
      if (found == characters.by_packet.end() ||
          found->second.vertices.empty() || found->second.draws.empty()) {
        complete = false;
        break;
      }
      const auto& owned = found->second;
      write(item.record);
      write(item.vertex_descriptor);
      write(item.vertex_address);
      write(item.vertex_size);
      write(item.packet_physical);
      write(item.bucket_entry);
      write(uint32_t(owned.vertices.size()));
      write(uint32_t(owned.draws.size()));
      encoded.insert(encoded.end(), owned.vertices.begin(), owned.vertices.end());
      for (const auto& draw : owned.draws) {
        if (!draw.final_seen || !sequences.insert(draw.sequence).second) {
          complete = false;
          break;
        }
        write(draw.sequence);
...
void PinyonShiftObservePresentationViewEnd() {
  ...
  if (scope.ordinal == 8 && Snr03ProbeSourceFrame(frame)) {
    if (snr03_scene_overflow || snr03_vegetation_items.empty() ||
      ...
      snapshot.characters = std::move(snr03_character_items);
      std::shared_ptr<const Snr03SceneSnapshot> scene =
      ...
      uint64_t dropped_frame = 0;
      {
        ...
        if (snr03_scenes.size() == 2) {
          ...
          snr03_character_payloads.erase(dropped_frame);
        }
      ...
      REXGPU_INFO("FH1 SNR03 scene published frame={} view={} camera={} "
                  "items={} fingerprint={}", frame, scene->view,
                  scene->camera, scene->items.size(), fingerprint);
      REXGPU_INFO("FH1 SNR03 character scene published frame={} items={}",
                  frame, scene->characters.size());
      ...
      for (size_t i = 0; i < scene->characters.size(); ++i) {
        const auto& item = scene->characters[i];
        REXGPU_INFO("FH1 SNR03 character item {{\"frame\":{},\"ordinal\":{},"
                    "\"record\":{},\"vertex_descriptor\":{},"
                    "\"vertex_address\":{},\"vertex_size\":{},"
                    "\"packet_physical\":{},\"bucket_entry\":{}}}",
                    frame, i + 1, item.record, item.vertex_descriptor,
                    item.vertex_address, item.vertex_size,
                    item.packet_physical, item.bucket_entry);
      }
    }
  }
  if (scope.ordinal == 8 && Snr03ProbeSourceFrame(frame)) {
    ...
    for (const auto& record : snr03_manager_title_records) {
      valid &= record.packet && packets.insert(record.packet).second;
    }
    if (valid) {
      Snr03ManagerScene snapshot{};
      snapshot.source_frame = frame;
      snapshot.view = scope.view;
      snapshot.camera = scope.camera;
      for (uint32_t i = 0; i < 16; ++i) {
      ...
      snapshot.records = std::move(snr03_manager_title_records);
      const auto scene = std::make_shared<const Snr03ManagerScene>(
          std::move(snapshot));
      {
        std::lock_guard lock(snr03_scene_mutex);
        ...
        snr03_manager_scenes[frame] = scene;
      }
      REXGPU_INFO("FH1 SNR03 manager scene published frame={} records={}",
  ...
  if (scope.ordinal == 8 && Snr03ProbeSourceFrame(frame)) {
    if (!snr03_car_title_rejected && !snr03_car_title_records.empty() &&
      ...
      snapshot.camera = scope.camera;
      for (uint32_t i = 0; i < 16; ++i) {
        snapshot.camera_matrices[i] =
            SnrM02ReadU32(scope.camera + 80 + i * 4);
        snapshot.camera_matrices[16 + i] =
            SnrM02ReadU32(scope.camera + 144 + i * 4);
      }
...
void PinyonShiftObserveSecondDrawEnd() {
  ...
  if (scope.target == 0x8245AB88 &&
      ...
      !snr01_view_scopes.empty() && snr01_view_scopes.back().ordinal == 8) {
    if (scope.packet_count != 1 || !scope.packet_physical ||
        !scope.bound_record || !scope.bound_vertex_descriptor ||
        !scope.bound_vertex_address || !scope.bound_vertex_size ||
        snr03_character_items.size() >= 64) {
      snr03_scene_overflow = true;
    } else {
      snr03_character_items.push_back({0, scope.bound_record,
          scope.bound_vertex_descriptor, scope.bound_vertex_address,
          scope.bound_vertex_size, scope.packet_physical, scope.bucket_entry});
    }
  }
  if (scope.ordinal <= kSnr01ProceduralLimit) {
    REXGPU_INFO(
...
void PinyonShiftObserveSnr01DirectFamilyRecord(
  ...
  if (Snr03ProbeSourceFrame(scope.frame) && scope.view_call == 8) {
    ...
    if (!r29.u32 || !r27.u32 ||
        ...
        std::any_of(snr03_manager_title_records.begin(),
                    snr03_manager_title_records.end(), [&](const auto& row) {
          return row.next_direct == next_direct;
        })) {
      snr03_manager_title_rejected = true;
    } else {
      snr03_manager_title_records.push_back({
          next_direct, r29.u32, r27.u32, 0,
          {SnrM02ReadU32(r29.u32), SnrM02ReadU32(r29.u32 + 4),
           SnrM02ReadU32(r29.u32 + 8), SnrM02ReadU32(r29.u32 + 16)},
          {SnrM02ReadU32(r27.u32), SnrM02ReadU32(r27.u32 + 12),
           SnrM02ReadU32(r27.u32 + 20)}});
    }
  }
  REXGPU_INFO(
